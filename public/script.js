...
function handleFileSelect(event) {
  ...
  if (fileInput.files.length > 0) {
    ...
    if (nameLower.endsWith('.csv')) {
      ...
      // Clear previous probabilities and inform the user to upload a CSV for genetic analysis
...
/**
 * Compute phenotype probabilities based on a sample's genetic marker data. The
 * sample argument is an object mapping genetic column names (e.g.
 ...
 *
 * @param {Object} sample - key/value pairs of genetic marker column and genotype
...
/**
 * Parse a CSV file containing genetic marker data. The CSV must have a
 * header row and at least one data row. Column names should match the
 * genetic identifiers used in the HIrisPlex-S system (e.g. 'rs12913832_T').
...
function loadDemo(){
  ...
  // Show a placeholder file name for demo genetic CSV
  document.getElementById('vcfFileInfo').textContent = 'demo_genetic_markers.csv (0.1 MB)';
...
function predict(){
  ...
  if (!state.hasPrediction) {
    alert(state.lang === 'en' ? 'Please upload a genetic data CSV file or load demo/random data first.' : 'يرجى رفع ملف CSV يحتوي على البيانات الوراثية أو تحميل بيانات تجريبية/عشوائية أولاً.');
    return;
  }
  showLoading();
...
async function generateFace() {
  ...
  // إزالة أية معالجة سابقة
  faceImg.onload = () => {
    // بمجرد تحميل الصورة بنجاح، إخفاء عنصر SVG والنص البديل
